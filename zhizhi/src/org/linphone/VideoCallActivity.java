...
public class VideoCallActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		previousPhoneOrientation = AndroidCameraRecordManager.getInstance()
				.getPhoneOrientation();
		phoneOrientation = 90 * getWindowManager().getDefaultDisplay()
				.getOrientation();
		...
		if (Version.sdkBelow(8)) {
			...
			setRequestedOrientation(recordManager.isCameraOrientationPortrait() ? ActivityInfo.SCREEN_ORIENTATION_PORTRAIT
	...
	private void updateCallIfOrientationChanged() {
		if (Version.sdkAbove(8) && previousPhoneOrientation != phoneOrientation) {
			...
			// camera will be restarted when mediastreamer chain is recreated
			// and setParameters is called

			// Base capture frame on streamed dimensions and orientation.
			resizeCapturePreview(mVideoCaptureView, LinphoneService.getLc()
	...
	/**
	 * @param sv
	 *            capture surface view to resize the layout
	 * @param vs
	 *            video size from which to calculate the dimensions
	 */
	private void resizeCapturePreview(SurfaceView sv, VideoSize vs) {
		...
		// if (Math.abs((newRatio-previewRatio)/newRatio) < similarRatio)
		// return;
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.videocall_menu_change_resolution:
			...
			VideoSize newVideoSize = LinphoneService.getLc()
					.getPreferredVideoSize();
