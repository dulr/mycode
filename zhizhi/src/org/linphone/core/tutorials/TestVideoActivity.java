 ...
 * 
 */
public class TestVideoActivity extends Activity implements Callback,
		OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		changeOrientation
				.setText(orientationToString(!currentOrientationIsPortrait));
		changeOrientation.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				if (width == 0 || height == 0)
					return;
				int newWidth = currentOrientationIsPortrait ? Math.min(height,
						width) : Math.max(height, width);
				int newHeight = currentOrientationIsPortrait ? Math.max(height,
						width) : Math.min(height, width);
				changeSurfaceViewLayout(newWidth, newHeight); // will change
																// width and
																// height on
																// surfaceChanged
			}
