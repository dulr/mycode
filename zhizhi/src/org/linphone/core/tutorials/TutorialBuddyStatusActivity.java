...
/**
 * Activity for displaying and starting the BuddyStatus example on Android
 * phone.
 * 
...
public class TutorialBuddyStatusActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		final TutorialNotifier notifier = new AndroidTutorialNotifier(mHandler,
				outputText);
	...
	private class TutorialLaunchingThread extends Thread {
		...
		public void run() {
			super.run();
			try {
				String myIdentity = mySipAddressWidget.getText().length() > 0 ? mySipAddressWidget
						.getText().toString() : null;
				String myPassword = mySipPasswordWidget.getText().length() > 0 ? mySipPasswordWidget
						.getText().toString() : null;
				tutorial.launchTutorial(sipAddressWidget.getText().toString(),
						myIdentity, myPassword);
