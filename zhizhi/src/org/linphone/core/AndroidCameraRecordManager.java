...
public class AndroidCameraRecordManager {
	...
	public void setPhoneOrientation(int degrees) {
		this.phoneOrientation = degrees;
	}
	...
	private AndroidCameraRecordManager() {
		cc = Version.sdkAbove(9) ? new AndroidCameraConf9()
				: new AndroidCameraConf();
	...
	public void setUseFrontCamera(boolean value) {
		if (cc.isFrontCamera(cameraId) == value)
			return; // already OK
	...
	public boolean isUseFrontCamera() {
		return cc.isFrontCamera(cameraId);
	}
	...
	public void setParametersFromFilter(long filterDataPtr, int height,
			int width, float fps) {
		stopVideoRecording();
	...
	private void tryToStartVideoRecording() {
		if (muted || surfaceView == null || parameters == null)
			return;
	...
	public List<Size> supportedVideoSizes() {
		...
		if (Version.sdkAbove(5)) {
			supportedVideoSizes = AndroidCameraRecord5Impl
					.oneShotSupportedVideoSizes();
		}
	...
	private int bufferRotationForCorrectImageOrientation() {
		...
		final int rotation = Version.sdkAbove(8) ? (360 - cameraOrientation + 90 - phoneOrientation) % 360
				: 0;
		Log.d(tag, "Capture video buffer will need a rotation of " + rotation
				+ " degrees : camera " + cameraOrientation + ", phone "
				+ phoneOrientation);
		return rotation;
	}
}
