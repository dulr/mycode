...
import org.linphone.zhizhiui.StartupActivity;
...
public class LinphonePreferencesActivity extends PreferenceActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (LinphoneService.isready()) {
			...
			enableIlbc = LinphoneService.instance().getLinphoneCore()
					.findPayloadType("iLBC", 8000) != null ? true : false;
			mIsLowEndCpu = !enableIlbc;
			if (!mIsLowEndCpu
					&& !getPreferenceManager().getSharedPreferences().contains(
			...
			if (mIsLowEndCpu) {
				getPreferenceManager()
						.getSharedPreferences()
						.edit()
						.putBoolean(getString(R.string.pref_codec_ilbc_key),
								false).commit();
				getPreferenceManager()
						.getSharedPreferences()
						.edit()
						.putBoolean(getString(R.string.pref_codec_speex16_key),
								false).commit();
				getPreferenceManager()
						.getSharedPreferences()
						.edit()
						.putBoolean(getString(R.string.pref_codec_speex32_key),
								false).commit();
			}
		...
		if (!mIsLowEndCpu) {
			getPreferenceScreen().findPreference(
					getString(R.string.pref_codec_ilbc_key)).setEnabled(
					enableIlbc);
			getPreferenceScreen().findPreference(
					getString(R.string.pref_codec_speex16_key)).setEnabled(
					enableIlbc);
		...
		if (getPreferenceManager().getSharedPreferences().getBoolean(
				DialerActivity.PREF_FIRST_LAUNCH, true)) {
			if (!mIsLowEndCpu) {
				AlertDialog.Builder builder = new AlertDialog.Builder(this);
				builder.setTitle(R.string.ec_calibration_launch_message)
						.setCancelable(false)
						.setPositiveButton(getString(R.string.cont),
								new OnClickListener() {
									public void onClick(DialogInterface dialog,
											int which) {
										startEcCalibration(getPreferenceScreen()
												.findPreference(
														getString(R.string.pref_echo_canceller_calibration_key)));
									}
								}).create().show();
			...
			getPreferenceManager().getSharedPreferences().edit()
					.putBoolean(DialerActivity.PREF_FIRST_LAUNCH, false)
					.commit();
		}
	...
	private void startEcCalibration(Preference preference) {
		try {
			while (mAudioManager
					.getStreamVolume(AudioManager.STREAM_VOICE_CALL) != mAudioManager
	...
	private void disableCheckbox(int key) {
		getPreferenceManager().getSharedPreferences().edit()
				.putBoolean(getString(key), false).commit();
		CheckBoxPreference box = (CheckBoxPreference) getPreferenceScreen()
